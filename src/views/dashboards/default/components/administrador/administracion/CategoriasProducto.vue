<template>
  <v-container fluid class="py-6">
    <h1 class="text-h4 mb-6">Categorias de Productos</h1>
    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-alert v-if="!categoriaStore.loading && categoriaStore.categoriasProducto.length === 0" type="info" class="mb-4">
          No hay categorías disponibles
        </v-alert>

        <v-table v-if="categoriaStore.categoriasProducto.length > 0" class="elevation-1">
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="cat in categoriaStore.categoriasProducto" :key="cat.id">
              <td>{{ cat.nombre }}</td>
...
const crearCategoria = async () => {
  ...
  try {
    const res = await categoriaStore.createCategoriaProducto(form.value)
    if (res) {
