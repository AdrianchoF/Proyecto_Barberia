  ...
  <v-container fluid class="py-6">
    ...
    <v-card class="brand-card mb-6" elevation="2" rounded="xl">
      ...
      <div class="form-header">
        ...
        <div>
          <h2 class="header-title">Categorías de Productos</h2>
          <p class="header-subtitle">Añade y gestiona las categorías de los productos de la barbería</p>
        </div>
      </div>
    ...
    <v-card elevation="2" rounded="xl" class="brand-card">
      <div class="table-header">
        <div class="table-title">
          ...
          <v-chip class="ml-3" size="small" color="#ee6f38" variant="flat" text-color="white">
            {{ categoriaStore.categoriasProducto.length }}
          </v-chip>
        </div>
      ...
      <v-card-text class="pa-4">
        ...
        <div v-if="!categoriaStore.loading && categoriaStore.categoriasProducto.length === 0" class="empty-state">
        ...
        <div v-else class="cat-table-wrapper">
          <table class="cat-table">
            ...
            <tbody>
              <tr v-for="cat in categoriaStore.categoriasProducto" :key="cat.id" class="cat-row">
                <td>
    ...
    <v-dialog v-model="dialogEditar" persistent max-width="460">
      <v-card rounded="xl" elevation="8">
        ...
        <v-card-actions class="pa-4 pt-0 gap-2">
          <v-btn variant="outlined" rounded="lg" @click="dialogEditar = false" class="flex-grow-1">
            Cancelar
          </v-btn>
    ...
    <v-dialog v-model="dialogEliminar" persistent max-width="400">
      <v-card rounded="xl" elevation="8">
        ...
        <v-card-actions class="pa-4 pt-0 gap-2">
          <v-btn variant="outlined" rounded="lg" @click="dialogEliminar = false" class="flex-grow-1">
            Cancelar
          </v-btn>
...
defineOptions({ name: 'CategoriasProducto' })
...
const recargar = async () => { await categoriaStore.getCategoriasProducto() }

const crearCategoria = async () => {
  if (!validCrear.value) return
  try {
    await categoriaStore.createCategoriaProducto(form.value)
...
const guardarEdicion = async () => {
  ...
  try {
    await categoriaStore.updateCategoriaProducto(formEditar.value.id, { nombre: formEditar.value.nombre })
...
const eliminarCategoria = async () => {
  try {
    await categoriaStore.deleteCategoriaProducto(categoriaAEliminar.value.id)
...
.refresh-btn {
  ...
  background: #f8f8f8; color: #ee6f38;
